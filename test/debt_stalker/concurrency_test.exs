defmodule DebtStalker.ConcurrencyTest do
  ...
  describe "event dispatcher concurrency" do
    ...
    test "SKIP LOCKED prevents double-processing of events" do
      ...
      apps =
        for i <- 1..5 do
          {:ok, app} = Applications.create_application(
            Map.put(@valid_es_attrs, :full_name, "User #{i}")
          )
          app
        end
      ...
      {:ok, %{rows: [[event_count]]}} =
        SQL.query(DebtStalker.Repo, "SELECT COUNT(*) FROM application_events WHERE processed_at IS NULL", [])
      ...
      total_processed = Enum.sum(Enum.map(results, fn {:ok, count} -> count; _ -> 0 end))
      ...
      {:ok, %{rows: [[total_events]]}} =
        SQL.query(DebtStalker.Repo, "SELECT COUNT(*) FROM application_events WHERE processed_at IS NOT NULL", [])
